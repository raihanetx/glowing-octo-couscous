  ...
  try {
    ...
    try {
      ...
      if (backupData.orderItems && backupData.orderItems.length > 0) {
        ...
        results.orderItems = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} order items`)
      }

      // Delete reviews
      if (backupData.reviews && backupData.reviews.length > 0) {
        const deleted = await db.delete(reviews)
        results.reviews = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} reviews`)
      }

      // Delete related products
      if (backupData.relatedProducts && backupData.relatedProducts.length > 0) {
        const deleted = await db.delete(relatedProducts)
        results.relatedProducts = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} related products`)
      }

      // Delete product FAQs
      if (backupData.productFaqs && backupData.productFaqs.length > 0) {
        const deleted = await db.delete(productFaqs)
        results.productFaqs = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} product FAQs`)
      }

      // Delete product images
      if (backupData.productImages && backupData.productImages.length > 0) {
        const deleted = await db.delete(productImages)
        results.productImages = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} product images`)
      }

      // Delete variants
      if (backupData.variants && backupData.variants.length > 0) {
        const deleted = await db.delete(variants)
        results.variants = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} variants`)
      }

      // Delete orders
      if (backupData.orders && backupData.orders.length > 0) {
        const deleted = await db.delete(orders)
        results.orders = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} orders`)
      }

      // Delete products
      if (backupData.products && backupData.products.length > 0) {
        const deleted = await db.delete(products)
        results.products = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} products`)
      }

      // Delete coupons
      if (backupData.coupons && backupData.coupons.length > 0) {
        const deleted = await db.delete(coupons)
        results.coupons = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} coupons`)
      }

      // Delete customers
      if (backupData.customers && backupData.customers.length > 0) {
        const deleted = await db.delete(customers)
        results.customers = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} customers`)
      }

      // Delete categories
      if (backupData.categories && backupData.categories.length > 0) {
        const deleted = await db.delete(categories)
        results.categories = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} categories`)
      }

      // Delete abandoned checkouts
      if (backupData.abandonedCheckouts && backupData.abandonedCheckouts.length > 0) {
        const deleted = await db.delete(abandonedCheckouts)
        results.abandonedCheckouts = { deleted: deleted.rowCount || 0, inserted: 0 }
        console.log(`[RESTORE] Deleted ${deleted.rowCount} abandoned checkouts`)
      }
