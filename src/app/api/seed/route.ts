...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete in reverse dependency order
    const productFaqsResult = await db.delete(productFaqs)
    const variantsResult = await db.delete(variants)
    const productsResult = await db.delete(products)
    const categoriesResult = await db.delete(categories)
    const couponsResult = await db.delete(coupons)
    ...
    return NextResponse.json({
      ...
      data: {
        productFaqs: productFaqsResult.rowCount || 0,
        variants: variantsResult.rowCount || 0,
        products: productsResult.rowCount || 0,
        categories: categoriesResult.rowCount || 0,
        coupons: couponsResult.rowCount || 0,
      }
    })
